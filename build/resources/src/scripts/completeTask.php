...
if (isset($_POST["id"])) {       //name of button
    ...
    $messages = array();
        ...
        if (!empty($_POST["id"])) {
            ...
            if ($taskCompleted=$completeController->completeTask($taskId)) {
                $taskCompletedFormated= (new DateTime($taskCompleted))->format('d / m / Y, H:i');
                array_push($messages, "Úloha bola úspešne dokončená.");
                echo json_encode(['code' => 200, 'msg' => $messages, 'time' =>$taskCompletedFormated], JSON_UNESCAPED_UNICODE);
                die();
